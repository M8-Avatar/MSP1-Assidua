...
class TauxCalculTest extends TestCase
{
    ...
    /** @test */
    public function taux_est_100_si_toutes_presences_sont_present(): void
    {
        $this->assertEquals(100.0, $this->calculerTaux(['present', 'present', 'present']));
    }

    /** @test */
    public function taux_est_100_si_aucune_presence_saisie(): void
    {
        $this->assertEquals(100.0, $this->calculerTaux([]));
    }

    /** @test */
    public function absences_justifiees_exclues_du_denominateur(): void
    {
        // 2 present + 1 absent_justifie → dénominateur = 2 → taux = 100 %
        $this->assertEquals(100.0, $this->calculerTaux(['present', 'present', 'absent_justifie']));
    }

    /** @test */
    public function absent_simple_compte_dans_le_denominateur(): void
    {
        // 1 present + 1 absent + 1 absent_justifie → dénominateur = 2 → taux = 50 %
        $this->assertEquals(50.0, $this->calculerTaux(['present', 'absent', 'absent_justifie']));
    }

    /** @test */
    public function retard_compte_comme_present_dans_le_numerateur(): void
    {
    ...
    /** @test */
    public function taux_de_25_pourcent_est_inferieur_au_seuil_alerte(): void
    {
        // 1 present / 4 total → 25 % → déclenche une alerte
    ...
    /** @test */
    public function taux_de_75_pourcent_exact_ne_declenche_pas_alerte(): void
    {
